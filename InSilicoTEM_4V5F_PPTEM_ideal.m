...
% 
% Milos Vulovic 2013
addpath('/home/philipp/software/dip/common/dipimage')
dip_initialise
...
params.proc.N               = 500; % Image size (field of view)
...
params.spec.source          = 'pdb';        % Options: 'map', 'pdb', or 'amorph'      
   params.spec.pdbin        = '4v6x' ;      % Required if params.spec.source = 'map' or 'pdb' ( 2GTL, 2WRJ, 1SA0, 1RYP or any other pdb entree)
...
params.mic.Cs          = 0;   % Spherical aberration  [m]
params.mic.a_i         = 0.050e-3; % Illumination aperture [rad]
params.mic.C_c         = 0;   % Chromatic aberration  [m]
params.mic.deltaE      = 0;      % Energy spread of the source [eV]
% ----------------------- aperture
params.mic.diam_obj          = 300e-6;   % Diameter of objective aperture [m]
...
params.mic.PP_qcuton      = 1/2000*1e10;% Cut-on frequency [1/m]. Typical values: 1/25, 1/75, 1/125, 1/250, 1/250


%----------------------- Acquisition settings (acquis field)
params.acquis.pixsize        = 2e-10;   % Pixel size in the specimen plane [m]3748
params.acquis.df             = [0]*1e-9; % Defocus [m]. Note: undefocus df>0; overfocus df <0. 
...
params.acquis.dose_on_sample = [5]/length(params.acquis.tilt); % Integrated flux (for the full tilt series) [e-/A2]
...
fn = '/home/philipp/drop/Philipp/mypapers/lowdose/data/figure4_snr_frc/4v6x/4V6X_300_PP-TEM_ideal.h5';
params2.fn = fn;
h5create(fn,'/dose',1);
