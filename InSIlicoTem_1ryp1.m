...
params.proc.N               = round(150 * 1.17647058824,0); % Image size (field of view)
...
params.inter.type           = 'ms'; % Options: 'pa' (projection), 'wpoa' (weak-phase), 'pa+wpoa', 'tpga'(thick-phase grating), 'ms'(multislice) 
    params.inter.msdz       = 1.5e-9; % Approximate thickness of the slice for multislice [m] Required if params.inter.type = 'ms'
...
params.acquis.dose_on_sample = [200]/length(params.acquis.tilt); % Integrated flux (for the full tilt series) [e-/A2]
...
% dipshow((InputVol(:,:,20)));
fn = '/home/philipp/1RYP_300_1p6A_61111.h5';
params2.fn = fn;
