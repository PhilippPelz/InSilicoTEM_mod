...
if df_run==0 && ast==0
    ...
    xdot = x*cos(0) - y*sin(0);
    ydot = x*sin(0) + y*cos(0);
    q = q + sqrt((xdot/inratioqlq ).^2+(ydot*inratioqqs).^2)*q_true_pix;
    qsym = qsym + sqrt((xdot).^2+(ydot).^2)*q_true_pix;
    c1= 2*pi*( 0.25*Cs*params2.inter.lambda^3*qsym.^4 - 0.5*df_run*params2.inter.lambda*q.^2);
    c = cos(c1)-1i*sin(c1);
    if isfield(c, 'data')
       bla = c.data;
       c = dip_image(bla);
    ...
    % damping envelopes:
    H = Cc* params2.mic.deltaE/params2.acquis.Voltage;
    ...
    nums = (pi*Cs*params2.inter.lambda^2.*qsym.^3-pi*df_run.*q).^2*params2.mic.a_i^2;
    Ks = exp(-nums/log(2));
    if isfield(Ks, 'data')
       bla=Ks.data;
       Ks=dip_image(bla);
...
if params2.disp.ctf 
    ...
    ylabel('Ideal Contrast transfer function');
