...
for ii=1:size(rpdb.X,2)
    xcoord    = [xcoord rpdb.X(ii)];
    ycoord    = [ycoord rpdb.Y(ii)];
    zcoord    = [zcoord rpdb.Z(ii)];
    eltype    = [eltype strjoin(rpdb.element(ii))];
%     fprintf('Element: %s\n',eltype(ii))
    tempfact  = [tempfact rpdb.b_factor(ii)];
    occfact = [occfact rpdb.occupancy(ii)];
    [~,~,m(ii)] = parametrizeScFac(eltype(ii));
